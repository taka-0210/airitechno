...
if (count($segments) === 2 && $segments[0] === 'products') {
    ...
    $sql = 'SELECT shop_id,jan13,status,date,rank,class1,maker,class2,class2_id,shohin,model,year,w,d,h,price2,price2_notax,stock,date_out,status2,ask,flag_close,tokucho,comment,hosyo FROM stock_list_rubs WHERE jan13 = :jancode AND shop_id IN (' . implode(',', $placeholders) . ') AND status2 = :public_status AND COALESCE(flag_close, 0) = 0 LIMIT 1';
    $statement = api_db()->prepare($sql);
    $statement->execute($parameters);
    $row = $statement->fetch();
    if (!$row) {
