...
商品APIの`videos`には、商品番号に紐づく個体動画だけを`type: individual`付きで返します。個体動画の拡張子は`.mp4` / `.MP4` / `.mov` / `.MOV`に対応します。型式マスターの`movie_url`は公開APIへ出力しません。厨房君で個体動画を登録し、商品を「HP掲載」にすると、その処理から呼ばれる即時差分同期後に新サイトへ商品情報が反映されます。個体動画ファイルが存在する場合は、同じ商品APIレスポンスで動画も表示されます。5分間隔のCRONは即時同期失敗時の保険です。

動画一覧には`GET /catalog/video-products`を使用します。公開商品全体から個体動画ファイルが存在する商品だけを返すため、通常の商品一覧APIのページング順には左右されません。
