...
try {
    if ($mode === 'full' || !is_file($livePath) || snapshot_schema_version($livePath) < 2) full_sync($source, $livePath, $previousPath, $cacheDirectory);
...
{
    $db->exec('CREATE TABLE products (item_data_no INTEGER NOT NULL UNIQUE,id TEXT PRIMARY KEY,store_id TEXT,store_name TEXT,status_no INTEGER,status_label TEXT,rank_no INTEGER,rank_name TEXT,year INTEGER,size_w INTEGER,size_d INTEGER,size_h INTEGER,price_sales INTEGER,flag_ask_price INTEGER,comment TEXT,date_ship TEXT,datetime_stock TEXT,datetime_update TEXT,count_photo INTEGER,cmn_photo_no INTEGER,warranty_enabled INTEGER,warranty_special_period TEXT,name TEXT,supple TEXT,maker TEXT,model TEXT,details TEXT,movie_url TEXT,class1_id INTEGER,class1_name TEXT,class2_id INTEGER,class2_name TEXT)');
