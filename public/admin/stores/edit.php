...
<div class="actions"><button type="submit">保存する</button><?php if ($id > 0): ?><a target="_blank" href="../../store.php?slug=<?= rawurlencode($store['slug']) ?>&amp;preview=1"><?= $store['status'] === 'published' ? '公開ページを見る' : '下書きをプレビュー' ?></a><?php endif; ?></div></form><?php render_admin_footer($user);
